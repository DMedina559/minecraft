...
execute as @initiator[scores={Moneyz=250..},tag=!w1003] run tag @s add tmp

execute as @initiator[scores={Moneyz=250..},tag=!w1003,tag=tmp] run scoreboard players remove @s Moneyz 250

execute as @initiator[tag=!w1003,tag=tmp] run tag @s add w1003

execute as @initiator[tag=w1003,tag=tmp] run tag @s remove tmp
