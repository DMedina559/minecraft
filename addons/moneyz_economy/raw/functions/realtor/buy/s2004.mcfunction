...
execute as @initiator[scores={Moneyz=500..},tag=!s2004] run tag @s add tmp

execute as @initiator[scores={Moneyz=500..},tag=!s2004,tag=tmp] run scoreboard players remove @s Moneyz 500

execute as @initiator[tag=!s2004,tag=tmp] run tag @s add s2004

execute as @initiator[tag=s2004,tag=tmp] run tag @s remove tmp
