...
execute as @initiator[scores={Moneyz=500..},tag=!s2003] run tag @s add tmp

execute as @initiator[scores={Moneyz=500..},tag=!s2003,tag=tmp] run scoreboard players remove @s Moneyz 500

execute as @initiator[tag=!s2003,tag=tmp] run tag @s add s2003

execute as @initiator[tag=s2003,tag=tmp] run tag @s remove tmp
